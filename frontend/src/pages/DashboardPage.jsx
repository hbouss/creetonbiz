...
export default function DashboardPage() {
  ...
  // Wake Lock (garder l’écran allumé pendant la génération)
  ...
  async function handleBuyInfinity() {
    try {
  ...
  async function handleBuyStartNow() {
    try {
      const { sessionId } = await createCheckoutSession("startnow");
      const stripe = await stripePromise;
      await stripe.redirectToCheckout({ sessionId });
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 overflow-x-hidden">
      ...
      >
        ...
        />
        <div
          className={`absolute right-0 top-0 h-full w-10/12 max-w-xs bg-gray-800 shadow-2xl border-l border-gray-700 transition-transform duration-200 ${menuOpen ? "translate-x-0" : "translate-x-full"}`}
        >
      ...
      <main className="p-4 sm:p-6 space-y-6">
        ...
        <section className="bg-gray-800 rounded-xl p-5 space-y-4">
          <div className="flex items-center justify-between">
            ...
            {ideas.length > 0 && <span className="text-xs text-gray-400">{ideas.length} idée(s)</span>}
          </div>
                ...
                <div key={stableId} className="bg-gray-700 p-4 rounded-lg flex flex-col gap-2">
                  <div className="flex justify-between items-start gap-3">
                    <div className="flex-1 min-w-0">
                      ...
                      {isLong && (
                        <button
                          type="button"
                          onClick={() => setExpandedIdeas(x => ({ ...x, [stableId]: !x[stableId] }))}
                    ...
                    {Number.isFinite(ratingNum) && (
                      <div className="shrink-0 flex flex-col items-end">
                        ...
                        <span className={`px-2 py-0.5 rounded text-xs text-white ${
                          ratingNum >= 8 ? "bg-emerald-600" : ratingNum >= 6 ? "bg-yellow-600" : "bg-gray-600"
                        }`}>🌟 {ratingNum.toFixed(1)} / 10</span>
                      </div>
                    )}
                  </div>
                  ...
                  {alreadyConverted && (
                    <div className="flex items-center gap-2 text-xs">
                      <span className="px-2 py-0.5 bg-emerald-700/40 text-emerald-300 rounded">Convertie en projet</span>
                      <button type="button" onClick={() => scrollToProject(firstProjectId)} className="text-emerald-300 hover:text-emerald-200 underline">
                        Voir le projet
                      </button>
                    </div>
                  )}
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2 sm:flex sm:gap-2">
                    ...
                    >
                      {alreadyConverted ? "Déjà convertie" : convertingIdeaId === i.id ? "Conversion…" : "Convertir"}
                    </button>
                    <button
                      disabled={loading}
